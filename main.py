...
async def write_to_db(connection, params):
    await connection.copy_records_to_table('incomeStatement', records=params)
        ...
        async with pool.acquire() as connection:
            async with aiohttp.ClientSession() as session:
                async with session.get(url=url) as response:
                    ...
                    params = [(company_id, response['retrievedDate'], response['symbol'], response['reportedCurrency'], response['fillingDate'], 
...
async def get_fins(pool, company_url):
    try:
        ret = await asyncio.gather(*[get_fin(pool, company_id, company_url[id]) for company_id in company_url])
        print(f"Finalized all. Return list of {len(ret)} outputs.")
    except Exception as e:
        print(e)

async def get_company_fin():
    pool = await asyncpg.create_pool(host=config.dbHost,port=config.dbPort,
                            user=config.dbUser,password=config.dbPassword,
                            database=config.dbName,command_timeout=60)
